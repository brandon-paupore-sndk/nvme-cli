...
static int get_additional_smart_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log(fd, cfg.namespace_id, 0xca, 0, false,
...
static int get_market_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log(fd, NVME_NSID_ALL, 0xdd, 0, false,
			   sizeof(log), log);
	if (!err) {
		if (!cfg.raw_binary)
...
static int get_temp_stats_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log(fd, NVME_NSID_ALL, 0xc5, 0, false,
			   sizeof(stats), &stats);
	if (!err) {
		if (!cfg.raw_binary)
...
static int get_lat_stats_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log(fd, NVME_NSID_ALL, cfg.write ? 0xc2 : 0xc1, 0,
